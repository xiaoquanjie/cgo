...
#include "rmdb/config.pb.h"
#include "rmdbclient.h"
#include "encode.h"
#include "algorithm.h"
#include <chrono>
#include <vector>
...
namespace rmdbclient {
    ...
    /////////////////////////////////////////////////////////////
    static std::unordered_map<uint32_t, RmdbClientPtr> gClientMap;

    static std::unordered_map<uint32_t, RmdbClientPtr>& getClientMap() {
        return gClientMap;
    }

    // 获取客户端.
    RmdbClientPtr GetClient(uint32_t id) {
        auto iter = gClientMap.find(id);
        if (iter == gClientMap.end()) {
            return nullptr;
        }
        return iter->second;
    }
    ...
    const char* CodeMsg(rmdb::ErrorCode code) {
        switch (code) {
            ...
            case rmdb::FAIL:
                return "fail";
            case rmdb::DATA_NOT_EXIST:
                return "data not exist";
            case rmdb::DATA_ALREADY_EXIST:
                return "data already exist";
            case rmdb::ERR_ARG:
                return "error argument";
            case rmdb::TABLE_NOT_EXIST:
                return "table not exist";
            case rmdb::ERR_DATA:
                return "error data can't be parse";
            case rmdb::ERR_MOVE:
                return "error move";
            case rmdb::ERR_OFFLINE:
                return "system offline";
            case rmdb::ERR_INDEX:
                return "illegal index";
            case rmdb::CLIENT_NOT_CONNECTED:
                return "client not connected";
            case rmdb::ERR_TABLE_NAME:
                return "illegal table name";
            case rmdb::DB_NOT_CREATED:
                return "db not created";
            case rmdb::TABLE_NOT_CREATED:
                return "table not created";
            case rmdb::ERR_DB_NAME:
                return "illegal db name";
            case rmdb::ERR_INDEX_MISMATCH:
                return "index mismatch";
            case rmdb::ERR_DATA_REACH_LIMIT:
                return "get too much data, reach limit";
            case rmdb::ERR_INDEX_DUPLICATED:
