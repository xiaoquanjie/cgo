...
public:
    ...
    */
    ...
    /**
    @brief try to inserts an item to the queue

    Only the owner thread can insert an item to the queue.
    The operation won't trigger the queue to resize its capacity
    if more space is fulled.
...
template <typename T>
template <typename O>
void WorkStealingQueue<T>::push(const O& o) {
    int64_t b = _bottom.load(std::memory_order_relaxed);
    int64_t t = _top.load(std::memory_order_acquire);
    Array* a = _array.load(std::memory_order_relaxed);
...
template <typename T>
template <typename O>
bool WorkStealingQueue<T>::try_push(O&& o) {
    int64_t b = _bottom.load(std::memory_order_relaxed);
    int64_t t = _top.load(std::memory_order_acquire);
    Array* a = _array.load(std::memory_order_relaxed);
