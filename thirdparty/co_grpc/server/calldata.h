...
public:
    ...
    ::grpc::Status On() {
        return oncall_(&ctx_, &req_, &rsp_);
    }

    void Finish(::grpc::Status status) {
        status_ = FINISH;
        responder_.Finish(rsp_, status, this);
    }
...
template<class Request, class Response, class Runner>
void ServerData<Request, Response, Runner>::Proceed(bool shutdown, bool ok) {
    if (shutdown) {
        delete this;
        return;
    }
...
public:
    ...
    // 不允许连续两次调用它，需等待callback回来后
    void AsyncWrite(const Response& rsp, std::function<void(bool)> on) {
        status_ = WRITING;
        writecall_ = on;
